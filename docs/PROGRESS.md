...
| T-019 Accounts & AI settings: Settings cog (Accounts & AI, Global rules, Hidden projects, About), per-project AI button, encrypted API keys, per-terminal env | in review | feat/accounts-settings | 24 unit checks; lead screenshots --view settings and --view ai: cog before the add button, four sections, live status for Claude Code, GitHub CLI, Codex (via ChatGPT app), Gemini not installed; env proof with a fake key (exported to a new terminal, gone after Remove, never plain on disk) |
