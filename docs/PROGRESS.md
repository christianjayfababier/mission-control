...
| T-019 Accounts & AI settings: provider registry, login flows, encrypted API keys, per-terminal env | in review | feat/accounts-settings | 22 unit checks (registry, assembleEnv, secrets with fake encryptor, status parsers); lead screenshot --view accounts: Claude Code, GitHub CLI and Codex (via ChatGPT app) detected live, Gemini not installed; env proof: fake key exported to a new terminal, absent after Remove, never plain on disk |
