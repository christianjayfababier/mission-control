...
- **Accounts & AI** (header button): the tools Mission Control and its workers run, in three groups. *Required* is Claude Code and the GitHub CLI — version, and who is logged in (`claude auth status`, `gh auth status`, probed with a 5 s timeout, 15 s for Claude, in parallel and cached for a minute). *AI tools* is the OpenAI Codex CLI and the Google Gemini CLI; Codex is also found at `%LOCALAPPDATA%\OpenAI\Codex\bin\<hash>\codex.exe`, where the ChatGPT desktop app puts it off PATH. *API keys* is the Anthropic, OpenAI and Gemini keys: they are encrypted with Electron's `safeStorage` (Windows DPAPI) into `~/.claude/mission-control/secrets.json` — the plain text never reaches the disk or the renderer — and exported into every **new** terminal of a project that may use them (`ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY`); a variable your own shell already exports is never overwritten. **Install** and **Log in** run the vendor's own command (winget, or `gh auth login -w`, or `claude auth login`) in a real terminal tab of the selected project, so you can answer its prompts; when it exits the probes run again. The Repo & account dialog carries the per-project half, *Providers this project may use* — everything is ticked until you untick it, and an unticked provider's key stays out of that project's terminals (`providers.js`, `secrets.js`, docs/ACCOUNTS-CONTRACT.md).
